...
#ifdef SENSOR_BOARD
extern uint8_t ucMasterBusIDLE;
#endif
...
static void vGetBaseInfo(uint8_t ucBaseType,uint8_t ucMotorType,float fRatio,float fWheelDiameter);
...
extern uint16_t usPosition1,usPosition2,usPosition3,usPosition4;
extern uint8_t ucVelocity1,ucVelocity2,ucVelocity3,ucVelocity4;
static void vSetLineActuator(uint8_t* pucCommandBuf);
#endif
...
extern uint8_t ucVelocity1,ucVelocity2,ucVelocity3,ucVelocity4;
static void vSetLineActuator(uint8_t* pucCommandBuf);
static void vGetLineActuator(uint8_t* pucCommandBuf);
#endif
/**
...
#ifdef SENSOR_BOARD
void vRecvSensorBoard(uint8_t* pucCommandBuf);
void vRecvRFIDTag(uint8_t* pucCommandBuf);
#endif

#ifdef CSTM
extern uint8_t ucLineFollowDir;
extern uint8_t ucLineFollowSpeed;
void vSetLineFollow(uint8_t* pucCommandBuf);
void vGetSonarSensor(uint8_t* pucCommandBuf);
void vGetTagInfo(void);
#endif
...
#ifdef SENSOR_BOARD
uint16_t usTrackSensor = 0;
uint16_t usSonarSensor = 0;
uint16_t pusTrackSensor[2] = {0};
uint16_t pusSonarSensor[2] = {0};
uint8_t pucRFIDTag[6] = {0};
#endif
...
uint8_t ucSaveCommandToCache(uint8_t ucCommandLen,uint8_t* pucCommandBuf){
	uint8_t pucTempBuf[ucCommandLen+1];
	memcpy(pucTempBuf,pucCommandBuf,ucCommandLen+1);
	...
	if(pucCommandBuf[2] == ucDeviceID){
		switch (pucCommandBuf[3]){
			...
			case 0x21:
				...
				vGetBaseInfo(ucBaseType,ucMotorType,(float)fRatio,fWheelDiameter);
				break;
			case 0x23:
				vGetRawImuCommand2();
				break;
...
void vSetVelocityCommand(uint8_t* pucCommandBuf){
	...
	fLineX = (float)(((int16_t)((pucDataBuf[0]<<8) + pucDataBuf[1]))/1000.0f);
	fLineY = (float)(((int16_t)((pucDataBuf[2]<<8) + pucDataBuf[3]))/1000.0f);
	fAngleZ = (float)((((int16_t)(pucDataBuf[4]<<8) + pucDataBuf[5]))/1000.0f);
//	if(bEmergencyStop){
...
static void vSetCarVelocityCommand(uint8_t* pucCommandBuf){
	...
	memcpy(pucDataBuf,pucCommandBuf+4,6);
	fLineX = (float)(((int16_t)((pucDataBuf[0]<<8) + pucDataBuf[1]))/1000.0f);
	fAccelX = (float)(((int16_t)((pucDataBuf[2]<<8) + pucDataBuf[3]))/1000.0f);
	fAngleZ = (float)((((int16_t)(pucDataBuf[4]<<8) + pucDataBuf[5]))/1000.0f);	
//	if(bEmergencyStop){
...
void vGetVelocityCommand(void){
	...
	sLinex = (int16_t)(xVelocity.fLineX*1000.0f);
	sLiney = (int16_t)(xVelocity.fLineY*1000.0f);
	sAngleZ = (int16_t)(xVelocity.fAngleZ*1000.0f);
...
void vGetBatteryInfoCommand(void){
	...
	usVoltage = (uint16_t)(xBattery.fVoltage*1000.0f);
	usCUrrent = (uint16_t)(xBattery.fCurrent*1000.0f);
	ucCommandSendBuf[0] = FRAME_HEAD;
...
#ifdef LINEAR_ACTUATOR
static void vSetLineActuator(uint8_t* pucCommandBuf){
	uint8_t pucDataBuf[12];
	int Encoder = 0;
	memcpy(pucDataBuf,pucCommandBuf+4,12);
	ucVelocity1 = pucDataBuf[0];
	ucVelocity2 = pucDataBuf[3];
	ucVelocity3 = pucDataBuf[6];
	ucVelocity4 = pucDataBuf[9];
	
	if((pucDataBuf[1] & pucDataBuf[2]) != 0xFF)
		usPosition1 = (int16_t)((int16_t)((pucDataBuf[1]<<8) + pucDataBuf[2]));
	if((pucDataBuf[4] & pucDataBuf[5]) != 0xFF)
		usPosition2 = (int16_t)((int16_t)((pucDataBuf[4]<<8) + pucDataBuf[5]));
	if((pucDataBuf[7] & pucDataBuf[8]) != 0xFF)
		usPosition3 = (int16_t)((int16_t)((pucDataBuf[7]<<8) + pucDataBuf[8]));
	if((pucDataBuf[10] & pucDataBuf[11]) != 0xFF)
		usPosition4 = (int16_t)((int16_t)((pucDataBuf[10]<<8) + pucDataBuf[11]));	
	...
	ucCommandSendBuf[1] = 18;
	ucCommandSendBuf[2] = ucDeviceID;
	ucCommandSendBuf[3] = 0x36;
	
	ucCommandSendBuf[4] = (uint8_t)abs(xMotor1.sCurrentRpm);
	Encoder = (int)(iGetEncoderValue(xMotor1.xEncoder)/10.5);
	if (Encoder < 0) Encoder = 0;
	ucCommandSendBuf[6] = (uint8_t)(Encoder);
	ucCommandSendBuf[5] = (uint8_t)(Encoder>>8);

	ucCommandSendBuf[7] = (uint8_t)abs(xMotor2.sCurrentRpm);
	Encoder = (int)(iGetEncoderValue(xMotor2.xEncoder)/10.5);
	if (Encoder < 0) Encoder = 0;
	ucCommandSendBuf[9] = (uint8_t)(Encoder);
	ucCommandSendBuf[8] = (uint8_t)(Encoder>>8);

	ucCommandSendBuf[10] = (uint8_t)abs(xMotor3.sCurrentRpm);
	Encoder = (int)(iGetEncoderValue(xMotor3.xEncoder)/10.5);
	if (Encoder < 0) Encoder = 0;
	ucCommandSendBuf[12] = (uint8_t)(Encoder);
	ucCommandSendBuf[11] = (uint8_t)(Encoder>>8);

	ucCommandSendBuf[13] = (uint8_t)abs(xMotor4.sCurrentRpm);
	Encoder = (int)(iGetEncoderValue(xMotor4.xEncoder)/10.5);
	if (Encoder < 0) Encoder = 0;
	ucCommandSendBuf[15] = (uint8_t)(Encoder);
	ucCommandSendBuf[14] = (uint8_t)(Encoder>>8);

	ucCommandSendBuf[16] = 0x00;
	ucCommandSendBuf[17] = crc8_chk_value(ucCommandSendBuf,17);
	vSendCommand(ucCommandSendBuf,ucCommandSendBuf[1]);
	
//	printf("vSetMotorVelocityCommand\r\n");
}
#endif
...
static void vGetLineActuator(uint8_t* pucCommandBuf){
	memcpy(ucCommandSendBuf,pucCommandBuf,pucCommandBuf[1]);
	vSendCommand(ucCommandSendBuf,ucCommandSendBuf[1]);
}
#endif
/**
...
*/
void vGetBaseInfo(uint8_t ucBaseType,uint8_t ucMotorType,float fRatio,float fDiameter){
	uint16_t Ratio = fRatio * 10;
	...
	ucCommandSendBuf[5] = ucMotorType;
	ucCommandSendBuf[6] = (uint8_t)(Ratio>>8);
	ucCommandSendBuf[7] = (uint8_t)(Ratio);
