...
void vEchoSonar(uint8_t ucGroup,GPIO_PinState IO ){
	...
	if(ucSonarGroupNum == 1){
		if(IO == GPIO_PIN_RESET){
			if(tnow<puiEnterVal[0]) uiGapus = ((uiTickNow-puiEnterTick[0])*1000 + (puiEnterVal[0] - tnow)/72);
			else uiGapus = ((uiTickNow-puiEnterTick[0]-1)*1000 + (SysTick->LOAD - tnow + puiEnterVal[0])/72);
			if(ucGroup == 1){
				uiSonarus[0] = uiGapus;
			}
			else if(ucGroup == 2){
		...
		if(IO == GPIO_PIN_RESET){
			//the divid value shuould be 72,but I do not know why use 72 will make a 1.17 scale error
			if(tnow<puiEnterVal[1]) uiGapus = ((uiTickNow-puiEnterTick[1])*1000 + (puiEnterVal[1] - tnow)/62);
			else uiGapus = ((uiTickNow-puiEnterTick[1]-1)*1000 + (SysTick->LOAD - tnow + puiEnterVal[1])/62);
			if(ucGroup == 1){
				uiSonarus[3] = uiGapus;
			}
			else if(ucGroup == 2){
